...
const (
	TelemetryService_StreamMetrics_FullMethodName = "/telemetry.v1.TelemetryService/StreamMetrics"
)
...
func (c *telemetryServiceClient) StreamMetrics(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[Metric, Ack], error) {
...
type TelemetryService_StreamMetricsClient = grpc.ClientStreamingClient[Metric, Ack]
...
func (UnimplementedTelemetryServiceServer) StreamMetrics(grpc.ClientStreamingServer[Metric, Ack]) error {
...
func _TelemetryService_StreamMetrics_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(TelemetryServiceServer).StreamMetrics(&grpc.GenericServerStream[Metric, Ack]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type TelemetryService_StreamMetricsServer = grpc.ClientStreamingServer[Metric, Ack]
